...
- The `send_test_alert` command prints a machine-readable `E2E_RESULT: {...}` JSON line that is recorded
  by Admin's E2E runner.
...
---------------------------------
For quick visual guidance, include these screenshots in `docs/runbook/assets/` and reference them here. Below are the recommended filenames and where they will appear in this document:

- `schedule_list_filters.png` — Admin `Schedule` list with filters and checkboxes visible (used in the "Admin: Quick steps" section).
- `schedule_bulk_action_menu.png` — The Action dropdown open showing the new override actions.
- `schedule_change_form.png` — The Schedule change form showing the blue `Per-schedule` notice and the override fields grouped together.

To add a screenshot, place PNG files in `docs/runbook/assets/` and commit them. The document will automatically display them once added.
