...
class ScheduleAdminActionsTests(TestCase):
    def setUp(self):
        ...
        pi1 = ProgramInstance.objects.create(
            program=program, scheduled_start=timezone.now(), scheduled_end=timezone.now() + timedelta(hours=1)
        )
        pi2 = ProgramInstance.objects.create(
            program=program,
            scheduled_start=timezone.now() + timedelta(days=1),
            scheduled_end=timezone.now() + timedelta(days=1, hours=1),
        )
        self.s1 = Schedule.objects.create(
            channel=self.channel, schedule_date=date.today(), program_instance=pi1, order=1
        )
        self.s2 = Schedule.objects.create(
            channel=self.channel, schedule_date=date.today(), program_instance=pi2, order=2
        )
